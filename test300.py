...
def print_keys():
    try:
        with open(metadata_file, "r") as f:
            metadata = json.load(f)
            print("{:<30} {:<30} {:<30}".format("Key", "Pool", "Container"))
            print("-" * 90)
            for entry in metadata:
                print("{:<30} {:<30} {:<30}".format(entry['key'], entry['pool'], entry['container']))
    except Exception as e:
        print(f"Error retrieving metadata: {e}")
...
def delete_key(key):
    try:
        ...
        keys_to_delete = [key for key in daos_dict if re.match(rf"{re.escape(key)}chunk\d+$", key)]
        if keys_to_delete:
            for key_to_delete in keys_to_delete:
                daos_dict.pop(key_to_delete)
            print(f"All keys with '{key}' removed from the container.")
        else:
            print(f"No keys found matching '{key}' in the container.")
        
        # Remove the entry from the metadata file
        with open(metadata_file, "r+") as f:
            metadata = json.load(f)
            metadata = [entry for entry in metadata if entry['key'] != key]
            f.seek(0)
            json.dump(metadata, f, indent=4)
            f.truncate()
            print(f"Entry with '{key}' removed from the metadata file.")
